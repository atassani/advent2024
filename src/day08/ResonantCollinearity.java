...
public class ResonantCollinearity {
    ...
    public static void main(String[] args) {
        ResonantCollinearity template = new ResonantCollinearity();
        template.run();
    }
    ...
    private Map<Character, List<Node>> getResonances(Map<Character, List<Node>> frequencyMap, int maxX, int maxY) {
        ...
        for (Map.Entry<Character, List<Node>> entry : frequencyMap.entrySet()) {
            ...
            List<Node> resonances = new ArrayList<>();
            ...
            for (int i = 0; i < nodes.size(); i++) {
                for (int j = i + 1; j < nodes.size(); j++) {
                    ...
                    Node resonance1 = new Node(node1.x - dx, node1.y - dy);
                    Node resonance2 = new Node(node2.x + dx, node2.y + dy);

                    if (isWithinBounds(resonance1, maxX, maxY)) {
                        resonances.add(resonance1);
                    }
                    if (isWithinBounds(resonance2, maxX, maxY)) {
